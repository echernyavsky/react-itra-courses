import { Listbox, ListboxItem } from "@nextui-org/react";
import ItemCounter from "./ItemCounter.tsx";
import routes from "../shared/constants/routes.ts";
import { useEffect, useState } from "react";
import { BeerCounters, getBeersCount } from "../shared/apis/beerApi.ts";
import { FormattedMessage } from "react-intl";

export default function Sidebar() {
  const [beerCounters, setBeerCounters] = useState<BeerCounters>({
    beersCount: 0,
    breweriesCount: 0,
  });

  useEffect(() => {
    (async () => {
      const counters = await getBeersCount();
      setBeerCounters(counters);
    })();
  }, []);

  return (
    <>
      <Listbox
        aria-label="User Menu"
        className="gap-0 divide-y divide-default-300/50 overflow-visible rounded-medium bg-content1 p-0 shadow-small dark:divide-default-100/80"
        itemClasses={{
          base: "px-3 first:rounded-t-medium last:rounded-b-medium rounded-none gap-3 h-12 data-[hover=true]:bg-default-100/80",
        }}
      >
        <ListboxItem key={routes.HOME} href={routes.HOME}>
          <FormattedMessage id="sidebar.home" />
        </ListboxItem>
        <ListboxItem
          key={routes.BREWERIES.ROOT}
          href={routes.BREWERIES.ROOT}
          endContent={<ItemCounter count={beerCounters.breweriesCount} />}
        >
          <FormattedMessage id="sidebar.breweries" />
        </ListboxItem>
        <ListboxItem
          key={routes.BEER_LIST.ROOT}
          href={routes.BEER_LIST.ROOT}
          endContent={<ItemCounter count={beerCounters.beersCount} />}
        >
          <FormattedMessage id="sidebar.beer-collection" />
        </ListboxItem>
        <ListboxItem key={routes.ROASTERS.ROOT} href={routes.ROASTERS.ROOT}>
          <FormattedMessage id="sidebar.roasters" />
        </ListboxItem>
        <ListboxItem key={routes.DRINKS.ROOT} href={routes.DRINKS.ROOT}>
          <FormattedMessage id="sidebar.drink" />
        </ListboxItem>
        <ListboxItem key={routes.MOVIES.ROOT} href={routes.MOVIES.ROOT}>
          <FormattedMessage id="sidebar.movies" />
        </ListboxItem>
      </Listbox>
    </>
  );
}
